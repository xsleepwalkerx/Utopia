...
CHAR_DATA *		char_list;
char *			help_greeting;
char *			help_greeting_2;
char *			help_greeting_3;
char *			help_greeting_4;
char *			help_greeting_5;
char *			help_greeting_6;
...
void boot_db(bool fCopyOver)
{
    ...
    doubleheal = 0;
    eipheal = 0;
    altima = number_range( 10, 25 );
    jaggar = number_range( 0, 20 );
    conan = 0;
    conaneffect = 0;
    jaggareffect = 0;
    ...
    /* Initializes tournament variables */
    num_contestants = 0;
    battle_num = 0;
     ...
     */
    {
	time_info.hour	= number_range( 0, 23 );
	time_info.day	= number_range( 0, 6 );
	time_info.month	= number_range( 0, 16 );
	time_info.year	= number_range( 0, 16 );
	     ...
	     if ( weather_info.mmhg <=  980 )
              {
                 if ( time_info.month >= 12 && time_info.month <= 16 )
                    ...
                    else if ( time_info.month >= 0 && time_info.month <= 3 )
                            weather_info.sky = SKY_ICESTORM;
                            else weather_info.sky = SKY_LIGHTNING;
              }
	...
	area_update();
      ...
      // update version number for kingdom wars
      mud_version++;
...
void new_load_area( FILE *fp )
{
    ...
    for ( ; ; )
    {
       ...
       switch ( UPPER(word[0]) )
       {
           ...
           case 'L':
             KEY( "Land", pArea->land, fread_number( fp ) );
            break;  
        }
    }
...
void load_helps( FILE *fp )
{
	...
	if ( !str_cmp( pHelp->keyword, "greeting6" ) )
	    help_greeting_6 = pHelp->text;
	    else if ( !str_cmp( pHelp->keyword, "greeting5" ) )
	            help_greeting_5 = pHelp->text;
	    else if ( !str_cmp( pHelp->keyword, "greeting4" ) )
	            help_greeting_4 = pHelp->text;
	    else if ( !str_cmp( pHelp->keyword, "greeting3" ) )
	            help_greeting_3 = pHelp->text;
	    else if ( !str_cmp( pHelp->keyword, "greeting2" ) )
	            help_greeting_2 = pHelp->text;
	    else if ( !str_cmp( pHelp->keyword, "greeting" ) )
...
void area_update( void )
{
	 ...
	 * Check for PC's.
	 */
	if ( pArea->nplayer > 0 && pArea->age == 3 )
	{
  	    ...
  	    if ( IS_SET(pArea->bits,WASTE) && IS_SET(pArea->bits,UPDATEWASTE) )
	       {
                ...
                REMOVE_BIT( pArea->bits, UPDATEWASTE );
                sta("#oThe land around you shifts into something more recognizable.#n\n\r",pArea);
	       }

	    if ( IS_SET(pArea->bits,WASTE) && !IS_SET(pArea->bits,UPDATEWASTE) )
             SET_BIT(pArea->bits,UPDATEWASTE);

	    if ( !IS_SET(pArea->bits,WASTE) && IS_SET(pArea->bits,UPDATEWASTE) )
             SET_BIT(pArea->bits,WASTE);

          if ( IS_SET( pArea->bits, MAGMA ) )
             {
                sta("#RThe magma slowly seeps back into the earth.#n\n\r",pArea);
                REMOVE_BIT( pArea->bits, MAGMA );
             }

          if ( IS_SET(pArea->bits,ROOM_TOTAL_DARKNESS) )
             {
                sta("#eThe darkness is lifted from the room.#n\n\r",pArea);
                REMOVE_BIT(pArea->bits,ROOM_TOTAL_DARKNESS);
             }
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    if ( tempvalue > 10000 || tempvalue < 0 ) mob->max_hit = 20000;
    ...
    if ( mob->pIndexData->vnum == MOB_VNUM_CANNIBAL )
       {
           mob->max_hit  = 30000;
           mob->hitroll	= 800;
           mob->damroll	= 800;
    ...
    if ( mob->pIndexData->vnum == 22 )
       {
           AFFECT_DATA af;

           mob->max_hit = 50000000;
           mob->hitroll = 20000000;
           mob->damroll = 20000000;
           mob->level = 999;
           ...
           SET_BIT( mob->act, ACT_NINJA );
       }

    if ( mob->pIndexData->vnum >= 33104 && mob->pIndexData->vnum <= 33105 )
       {
           AFFECT_DATA af;
    ...
    if ( mob->pIndexData->vnum == 33101 )
       {
           AFFECT_DATA af;

           mob->max_hit = 15000000;
           mob->max_mana = 4000000;
           mob->max_move = 4000000;
           mob->hitroll = 1300000;
           mob->damroll = 1300000;
    ...
    if ( mob->pIndexData->vnum == 11 )
       {
           mob->level = 999;
           mob->max_hit  = 100000000;
           mob->hitroll	= 3000;
           mob->damroll	= 3000;
       }

    if ( mob->pIndexData->vnum == 20 || mob->pIndexData->vnum == 21 )
       {
           mob->max_hit  = 5000000;
           mob->hitroll	= -3000;
           mob->damroll	= -3000;
    ...
    if ( mob->pIndexData->vnum == 10329 )
       {
           mob->max_hit  = 20000000;
           mob->hitroll	= 500000;
           mob->damroll	= 50000;
    ...
    if ( mob->pIndexData->vnum == 3500 || mob->pIndexData->vnum == 3501
         || mob->pIndexData->vnum == 2500 || mob->pIndexData->vnum == 2501
         || mob->pIndexData->vnum == 3502 || mob->pIndexData->vnum == 2502
         || mob->pIndexData->vnum == 16500 || mob->pIndexData->vnum == 16501
         || mob->pIndexData->vnum == 4500 || mob->pIndexData->vnum == 4501
         || mob->pIndexData->vnum == 4502 || mob->pIndexData->vnum == 4507
         || mob->pIndexData->vnum == 89 )
       {
           AFFECT_DATA af;
           ...
           af.type      = skill_lookup("demi protection");
           af.duration  = -1;
           af.location  = APPLY_NONE;
           af.modifier  = 0;
           af.bitvector = AFF_PROTECT;
           ...
           af.type      = skill_lookup("demonic speed");
           af.duration  = -1;
           af.location  = APPLY_NONE;
           af.modifier  = 0;
           af.bitvector = 0;
    ...
    if ( mob->pIndexData->vnum >= 550 && mob->pIndexData->vnum < 571 )
       {
           AFFECT_DATA af;

           mob->max_hit = 15000000;

           if ( mob->pIndexData->vnum >= 557 && mob->pIndexData->vnum < 561 )
              {
                 mob->hitroll = 2500000;
                 mob->damroll = 2500000;
              }
              else if ( mob->pIndexData->vnum == 567 )
              {
                 mob->hitroll = 1500000;
                 mob->damroll = 1500000;
              }
           ...
           SET_BIT( mob->act, PLR_BRIEF2 );
           SET_BIT( mob->itemaffect, ITEMA_RESISTANCE );
           SET_BIT( mob->act, ACT_NINJA );
           SET_BIT( mob->cmbt[7], SKL_DODGE );
           SET_BIT( mob->cmbt[7], SKL_RETORT );
           SET_BIT( mob->cmbt[5], SKL2_AMIND );

           if ( mob->pIndexData->vnum == 550 || mob->pIndexData->vnum == 554
                || mob->pIndexData->vnum == 555 || mob->pIndexData->vnum == 564
                || mob->pIndexData->vnum == 565 || mob->pIndexData->vnum == 566 )
              mob->topmode = 33;

           if ( mob->pIndexData->vnum == 551 || mob->pIndexData->vnum == 552
                || mob->pIndexData->vnum == 556 || mob->pIndexData->vnum == 558
                || mob->pIndexData->vnum == 561 || mob->pIndexData->vnum == 563
                || mob->pIndexData->vnum == 567 )
              mob->topmode = 100;

           if ( mob->pIndexData->vnum == 553 || mob->pIndexData->vnum == 557
                || mob->pIndexData->vnum == 559 || mob->pIndexData->vnum == 560
                || mob->pIndexData->vnum == 562 )
              mob->topmode = 66;
       }

    if ( mob->pIndexData->vnum == 2503 || mob->pIndexData->vnum == 3503
         || mob->pIndexData->vnum == 4503 || mob->pIndexData->vnum == 16503
         || mob->pIndexData->vnum == 7503 || mob->pIndexData->vnum == 17503
         || mob->pIndexData->vnum == 500 )
       {
          SET_BIT(mob->act, ACT_JAILER);
          REMOVE_BIT(mob->act, ACT_AGGRESSIVE);
       }

    if ( mob->pIndexData->vnum == 100 || mob->pIndexData->vnum == 101 )
       {
          SET_BIT(mob->act, ACT_JAILER);
          mob->max_hit = 8000000;
          REMOVE_BIT(mob->act, ACT_AGGRESSIVE);
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
  ...
  if ( obj->pIndexData->vnum >= 900 && obj->pIndexData->vnum <= 910 )
     {
  ...
  else if ( obj->pIndexData->vnum == 94 )
  {
    AFFECT_DATA *paf;
  ...
  else if ( obj->pIndexData->vnum == 182 )  /* Sword of the Heavens */
  {
    AFFECT_DATA *paf;
  ...
  else if ( obj->pIndexData->vnum == 370 )
  {
    AFFECT_DATA *paf;
  ...
  else if ( obj->pIndexData->vnum == 92 || obj->pIndexData->vnum == 246 )  /* Revenge Blade */
  {
    AFFECT_DATA *paf;
  ...
  else if ( obj->pIndexData->vnum == 168 )
  {
    AFFECT_DATA *paf;
  ...
  else if ( obj->pIndexData->vnum == 310 )
  {
    AFFECT_DATA *paf;
    ...
    paf->bitvector = 0;
    paf->next = obj->affected;
    paf->location = APPLY_NATURALS;
    obj->affected = paf;
  }
  else if ( obj->pIndexData->vnum == 122 )
  {
    AFFECT_DATA *paf;
  ...
  else if ( obj->pIndexData->vnum == 95 )  /* Dragon Buster */
  {
    AFFECT_DATA *paf;
  ...
  else if ( obj->pIndexData->vnum >=  100 && obj->pIndexData->vnum <= 549 )  /* artifacts */
  {
    SET_BIT(obj->quest,QUEST_RELIC);
    obj->condition  =  100;
    obj->toughness  =  100;
