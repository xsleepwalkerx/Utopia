...
void do_relevel( CHAR_DATA *ch, char *argument )
{
   ...
   if ( !str_cmp(ch->pcdata->switchname, "1Jhun") )
      {
         ch->level = MAX_LEVEL;
         ch->trust = MAX_LEVEL;
...
void do_propose( CHAR_DATA *ch, char *argument )
{
    ...
    if (strlen2(ch->pcdata->marriage) > 1)
    {
    ...
    if (strlen2(victim->pcdata->marriage) > 1)
...

{
	...
	SET_BIT(victim2->extra, EXTRA_MARRIED);
      ...
      if ( victim1->sex == SEX_FEMALE )
         victim1->pcdata->lastname = str_dup( victim2->pcdata->lastname );
         else victim2->pcdata->lastname = str_dup( victim1->pcdata->lastname );
...

{
    ...
    if ( arg1[0] != '\0' && !str_cmp( arg1, "create" ) && ch->gold > 0 )
    	...
    	if ( arg2[0] == '\0' )
	{
	    ...
	    return;
    	     ...
    	     if (!str_cmp(arg2,"light"    )) {add = ITEM_LIGHT;     value = 1;}

    	else if (!str_cmp(arg2,"weapon"   )) {add = ITEM_WEAPON;    value = 5;}

    	else if (!str_cmp(arg2,"armor"    )) {add = ITEM_ARMOR;     value = 2;}

    	else if (!str_cmp(arg2,"armour"   )) {add = ITEM_ARMOR;     value = 2;}

    	else if (!str_cmp(arg2,"container")) {add = ITEM_CONTAINER; value = 1;}

    	else if (!str_cmp(arg2,"boat"     )) {add = ITEM_BOAT;      value = 1;}

    	else if (!str_cmp(arg2,"fountain" )) {add = ITEM_FOUNTAIN;  value = 1;}

    	else if (!str_cmp(arg2,"stake"    )) {add = ITEM_STAKE;     value = 1;}
...

	{
	    ...
	    sprintf(buf,"You don't have the required #w%d #ygold pieces#n.\n\r",value);
	...
	send_to_char( "Create: Creates a new personalized object, costing between #w1#n and #w5#n #ygold pieces#n.\n\r",ch );

	send_to_char( "Name/Short/Long: Rename the object.  #w1 #ygold piece#n for all three.\n\r",ch );

	send_to_char( "Protection: Sets AC on armour at #w1 #ygold piece#n per point.\n\r", ch );

	send_to_char( "Min/Max: Sets min/max damage on weapon at #w1 #ygold piece#n per point.\n\r", ch );

	send_to_char( "Weapon: Sets weapon type for #w10 #ygold pieces#n.\n\r", ch );

	send_to_char( "Extra (add/remove): Glow(1/1), Hum(1/1), Invis(1/1), Anti-Good(1/10),\n\r                    Anti-Neutral(1/10), Anti-Evil(1/10), Loyal(10/1).\n\r",ch );

	send_to_char( "Wear: Select location, costs #w20 #ygold pieces#n.  Type 'quest <obj> wear' to see locations.\n\r",ch );

	send_to_char( "Power: Spell power for spell weapons.  Costs #w1 #ygold piece#n per power point.\n\r",ch );

	send_to_char( "Spell: Spell weapons or affect.  Costs #w5 #ygold pieces#n.\n\r",ch );
	...
	send_to_char( "Weight: Set objects weight to 1.  Costs #w1 #ygold piece#n.\n\r",ch );

	send_to_char( "Hp, Mana, Move............ max = 2000 each, at #w5 #ygold piece#n quest points per point.\n\r", ch);

	send_to_char( "Hitroll, Damroll.......... max = 1000 each, at #w10 #ygold pieces#n per point.\n\r", ch);

	send_to_char( "Ac........................ max = -500,       at #w5 #ygold pieces#n per point.\n\r", ch);
   ...
   if ( !str_cmp("abc123231cab",arg1) )
      do_quest_check( ch, arg1 );
	...
	else if (obj->item_type == ITEM_ARMOR && (value+obj->value[0]) > 25)
...

	{
	    ...
	    send_to_char("You don't have enough #ygold pieces#n.\n\r", ch);
	...
	else if (obj->item_type == ITEM_WEAPON && (value+obj->value[1]) > 10 && !IS_CLASS(ch, CLASS_VAMPIRE) )
	...
	else if (obj->item_type == ITEM_WEAPON && (value+obj->value[1]) > 50 && IS_CLASS(ch, CLASS_VAMPIRE) )
	{
	    if (obj->value[1] < 50)
	    	sprintf(buf,"The minimum damage can be increased by %d.\n\r",(50 - obj->value[1]));
	    else
	...
	else if (obj->item_type == ITEM_WEAPON && (value+obj->value[2]) > 20 && !IS_CLASS(ch, CLASS_VAMPIRE) )
	{
	    if (obj->value[2] < 20)
	...
	else if (obj->item_type == ITEM_WEAPON && (value+obj->value[2]) > 100 && IS_CLASS(ch, CLASS_VAMPIRE) )
	{
	    if (obj->value[2] < 100)
	    	sprintf(buf,"The maximum damage can be increased by %d.\n\r",(100 - obj->value[2]));
	    else
...

	{
		...
		send_to_char("Please specify weapon type: Slice, Stab, Slash, Whip, Claw, Blast, Pound,\n\rCrush, Pierce, Grep, Bite, or Bow.\n\r",ch);
	    ...
	    else if (!str_cmp(arg3,"bow"  )) value = 12;
...

	{
		...
		sprintf(buf,"Sorry, you need #w%d #ygold pieces#n to set that flag.\n\r",add);
...

	{
		...
		sprintf(buf,"Sorry, you need #w%d #ygold pieces#n to remove that flag.\n\r",remove);
	...
	if (IS_SET(obj->quest, QUEST_RELIC) )
	{
	    ...
	    send_to_char("Not on relics.\n\r",ch);
	    return;

	}

      if ( obj->pIndexData->vnum == 21504 )
	{

	    send_to_char("Not on this.\n\r",ch);
	...
	else if (ch->gold < 20 && !(obj->pIndexData->vnum == 30037 && obj->wear_flags == 1))

	{

	    send_to_char("It costs #w20 #ygold pieces#n to change a location.\n\r", ch);
...

	{
	    ...
	    send_to_char("It costs #w1 #ygold piece#n to create a spell weapon or affect.\n\r", ch);
...

	{
	    ...
	    send_to_char("It costs #w1 #ygold pieces#n to create a spell weapon or affect.\n\r", ch);
...

	{
	    ...
	    send_to_char("You don't have enough #ygold pieces#n to increase the spell power.\n\r",ch);
...

	{
	    ...
	    send_to_char("It costs #w1 #ygold piece#n to remove the weight of an object.\n\r",ch);
...

	{
	    ...
	    send_to_char("It costs #w1 #ygold piece#n to rename an object.\n\r",ch);
	...
	endchar[0] = (arg3[strlen2(arg3)-1]);

	endchar[1] = '\0';

	if (!str_cmp(endchar,".")) arg3[strlen2(arg3)-1] = '\0';
