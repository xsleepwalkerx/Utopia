...
{ "Kingdom",	"Kingdom messages",		 0,	2,     "all", DEF_NORMAL,  20, NULL, FALSE}
};
...
static void load_board (BOARD_DATA *board)
{
    ...
    for ( ; ; )
    {
        ...
        sprintf( buf, fread_word( fp ) );

        if ( !str_cmp( buf, "kingdom" ) )
           {
               pnote->kingdom = fread_number( fp );
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
	...
	if ( note->kingdom > 0 && ch->pcdata->kingdom != note->kingdom )
         return FALSE;
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
      ...
      if ( !str_cmp( ch->pcdata->board->short_name, "Kingdom" ) )
         {
            if ( ch->pcdata->kingdom > 0 )
               kingdom = TRUE;
                       ...
                       send_to_char ("Non Kingdom members cannot post notes on this board.\n\r",ch);
                       return;
                    }
         }
	...
	if (!ch->pcdata->in_progress)
	{
		ch->pcdata->in_progress = new_note();
            ...
            if ( IS_IMMORTAL(ch) )
               ch->pcdata->in_progress->sender = str_dup ("Immortal");
               else ch->pcdata->in_progress->sender = str_dup (ch->pcdata->switchname);

            if ( kingdom )
               ch->pcdata->in_progress->kingdom = ch->pcdata->kingdom;

		/* convert to ascii. ctime returns a string which last character is \n, so remove that */	
		strtime = ctime (&current_time);
		strtime[strlen2(strtime)-1] = '\0';
...
void handle_con_note_text (DESCRIPTOR_DATA *d, char * argument)
{
	...
	if (strlen2 (buf) > MAX_LINE_LENGTH)
	{
      ...
      if ( colour_count > 20 )
         {
            write_to_buffer (d, "Due to abusive use, you pass the limit to the number of colours you can have!\n\r",0);
            write_to_buffer (d, "Please write the note over with less colours!\n\r",0);
		free_note (ch->pcdata->in_progress);
	...
	CHAR_DATA *ch = d->character;
		...
		switch (tolower(argument[0]))
		{
			...
			case 'p': /* post note */
				...
				/* remove AFK status */
				ch->pcdata->in_progress = NULL;
                        do_info( ch, "#wA #gdelicate #ym#Ca#Pi#Ll#g fairy pokes #wyou#g on the head whispering '#ca new note had just been posted.#g'#n");
