...
void improve_wpn( CHAR_DATA *ch, int dtype, int right_hand )
{
    ...
    if ( IS_CLASS(ch, CLASS_DEMON) && dtype != 0 )
       cur_skl = max_skl = UMAX( 200, UMIN( 2000, ch->pkill * 100 ) );

    if ( IS_CLASS(ch, CLASS_ASSASSIN) || IS_CLASS(ch, CLASS_SHADOW) )
       cur_skl = max_skl = 1500;

    if ( IS_CLASS(ch, CLASS_ASSASSIN) || IS_CLASS(ch, CLASS_SHADOW_WAR) )
       cur_skl = max_skl = 1000;

    if ( IS_CLASS(ch, CLASS_NINJA) )
       {
          if ( dtype == 11 && cur_skl < 1500 )
             max_skl = 1500;
             else if ( dtype == 1 && cur_skl < 1000 )
                     max_skl = 1000;
    ...
    if ( IS_CLASS(ch, CLASS_VAMPIRE) )
       {
          if ( dtype == 10 && cur_skl < 1500 )
             max_skl = 1500;
             else if ( dtype == 5 && cur_skl < 1000 )
                     max_skl = 1000;
    ...
    if ( IS_CLASS(ch, CLASS_SNOW_ELF) )
       {
          if ( dtype == 11 && cur_skl < 1500 )
             max_skl = 1500;
             else if ( dtype == 3 && cur_skl < 1000 )
                     max_skl = 1000;

          cur_skl = max_skl;
       }

    if ( IS_CLASS(ch, CLASS_MONK) || IS_CLASS(ch, CLASS_NIGHT_BLADE)
         || IS_CLASS(ch, CLASS_SAIYAN) )
       {
          if ( dtype == 0 && cur_skl < 1500 )
             max_skl = 1500;
    ...
    if ( IS_CLASS(ch, CLASS_WEREWOLF) )
       {
          if ( ( dtype == 0 || dtype == 11 || dtype == 5 || dtype == 2 ) && cur_skl < 1000 )
             max_skl = 1000;
    ...
    if ( IS_CLASS(ch, CLASS_PHOENIX) )
       {
          if ( ( dtype == 0 || dtype == 5 ) && cur_skl < 1500 )
             max_skl = 1500;
    ...
    if ( IS_CLASS(ch, CLASS_DROW) )
       {
          if ( dtype == 4 && cur_skl < 1500 )
             max_skl = 1500;
             else if ( dtype == 11 && cur_skl < 900 )
                     max_skl = 900;
    ...
    if ( ch->wpn[dtype] >= max_skl || ( ch->wpn[dtype] > 200 && number_percent() < 30 ) )
       return;
    ...
    else if (ch->wpn[dtype] == 100 ) sprintf(bufskill,"a master");
    ...
    else if (ch->wpn[dtype] == 1500 ) sprintf(bufskill,"ultimately skilled");
    else return;
...
void do_alignment( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->alignment >= 200 || ch->alignment <= -200 )
    {
	send_to_char( "You have chosen a path and can't change it until you detox.\n\r", ch );
	return;
    }

    if ( arg[0] == '\0' )
    {
	send_to_char( "Please enter 'good', or 'evil'.\n\r", ch );
	return;
    }
    ...
    else     send_to_char( "Please enter 'good', or 'evil'.\n\r", ch );
}
...
void do_skill( CHAR_DATA *ch, char *argument )
{
    ...
    else if (victim->wpn[dtype] == 1000) sprintf(bufskill,"ultimately skilled");
    else return;
    ...
    else if (victim->wpn[dtype2] == 1000) sprintf(bufskill2,"ultimately skilled");
    else return;
...
void critical_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, int dam)
{
    ...
    if ( !IS_NPC(victim) && IS_CLASS(victim,CLASS_DRAGON) && number_percent() < victim->pcdata->dragon_toughness * 2 )
       return;

    if ( !IS_NPC( victim ) && victim->pcdata->kingdom == 1 && victim->pcdata->clanrank > 1 )
       return;

    if ( IS_SET( ch->more2, MORE2_DRAGON ) )
       vorpal = TRUE;

    dtype = dt - 1000;
    if (dtype < 0 || dtype > 12) return;
    if (IS_NPC(ch)) critical += ((ch->level+1) / 5);
    if (!IS_NPC(ch)) critical += ((ch->wpn[dtype]+1)/30);
	...
	if (victim->wpn[wpn] > victim->wpn[wpn2])
	    critical -= ((victim->wpn[wpn]+1)/30);
	else
	    critical -= ((victim->wpn[wpn2]+1)/30);
    }

    if ( critical < 1 ) critical = 1;

    if ( critical > 3 ) critical = 3;
  
    if ( number_percent() <= 100 - critical )
       return;
	...
	if ( (damaged = get_eq_char(victim,WEAR_FACE) ) != NULL && !IS_ITEMAFF(ch, ITEMA_VORPAL) && !vorpal)
	{
	...
	if ( (damaged = get_eq_char(victim,WEAR_HEAD) ) != NULL && !IS_ITEMAFF(ch, ITEMA_VORPAL) && !vorpal)
	{
	...
	if ( (damaged = get_eq_char(victim,WEAR_ARMS) ) != NULL && !IS_ITEMAFF(ch, ITEMA_VORPAL) && !vorpal)
	{
	...
	if ( (damaged = get_eq_char(victim,WEAR_HANDS) ) != NULL && !IS_ITEMAFF(ch, ITEMA_VORPAL) && !vorpal)
	{
	...
	if ( (damaged = get_eq_char(victim,WEAR_LEGS) ) != NULL && !IS_ITEMAFF(ch, ITEMA_VORPAL) && !vorpal)
	{
	...
	if ( (damaged = get_eq_char(victim,WEAR_FEET) ) != NULL && !IS_ITEMAFF(ch, ITEMA_VORPAL) && !vorpal)
	{
	...
	if ( (damaged = get_eq_char(victim,WEAR_BODY) ) != NULL && !IS_ITEMAFF(ch, ITEMA_VORPAL) && !vorpal)
	{
...
void special_move( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( IS_CLASS(victim, CLASS_MONK) || IS_CLASS(victim, CLASS_NINJA) )
       return;
