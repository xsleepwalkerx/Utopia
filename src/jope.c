...
const struct jope_type jope_table [] =
{
  ...
  { "addaffect",    jope_addaffect,  12 },
  { "done",         jope_done,        7 },
  { "drop",         jope_drop,       10 },
  { "duration",     jope_duration,   12 },
  ...
  { "removeaffect", jope_removeaffect,  12 },
...
void jope_addaffect(CHAR_DATA *ch, char *argument)
{
   CHAR_DATA *victim;
   char arg[MAX_INPUT_LENGTH];
   int sn = -1;
   ...
   if ( is_affected(victim,sn) )
      {
         stc("They already have that spell on them!\n\r",ch);
         return;
      }

   (*skill_table[sn].spell_fun) ( sn, 100, victim, victim );
   stc("Ok.\n\r",ch);
}

void jope_removeaffect(CHAR_DATA *ch, char *argument)
{
   CHAR_DATA *victim;
   char arg[MAX_INPUT_LENGTH];
   int sn = -1;
...
void jope_gold(CHAR_DATA *ch, char *argument)
{
   CHAR_DATA *victim;
   char arg[MAX_INPUT_LENGTH];
   ...
   if ( !is_number(arg) )
      {
         send_to_char("Please use a number for the gold pieces.\n\r", ch);
         return;
      }

   victim->gold = UMIN( 500000000, UMAX( 0, atoi(arg) ) );
   send_to_char("Done.\n\r", ch);
}

void jope_duration(CHAR_DATA *ch, char *argument)
{
   CHAR_DATA *victim;
   char arg[MAX_INPUT_LENGTH];
   AFFECT_DATA *paf;
   ...
   if ( !is_number(arg) )
      {
         send_to_char("Please use a number for the duration.\n\r", ch);
         return;
      }
   ...
   if ( duration < -1 || duration > 30000 )
      {
         send_to_char("Duration range is -1 to 30000 ticks.\n\r", ch);
         return;
      }
...
void jope_spells(CHAR_DATA *ch, char *argument)
{
   ...
   value = atoi(arg2);
   if ( !str_prefix(arg1, "tempest") )
      victim->spl[0] = value;
      else if ( !str_prefix(arg1, "red") )
              victim->spl[1] = value;
      else if ( !str_prefix(arg1, "white") )
              victim->spl[2] = value;
...
void jope_show(CHAR_DATA *ch, char *argument)
{
   ...
   sprintf(line, "\n\r");
   sprintf(bufbanner, "#e<#r----------------#w==============#g{ J.O.P.E }#w==============#r----------------#e>#n\n\r");
   send_to_char(bufbanner, ch);
   send_to_char(line, ch);

   sprintf( buf, "  #CExp : %-10d   Gold : %-10d   Primal : %-10d   Level : %-2d   Trust : %-2d#n\n\r",
                 victim->exp, victim->gold, victim->practice, victim->level, victim->trust);
   send_to_char(buf, ch);

   sprintf( buf, "  #CHit : %d/%d   Move : %d/%d   Mana : %d/%d#n\n\r",
                 victim->hit, victim->max_hit, victim->move, victim->max_move, victim->mana, victim->max_mana);
   send_to_char(buf, ch);

   sprintf( buf, "  #CPkill : %d   Pdeath : %d   Mkill : %d   Mdeath : %d#n\n\r",
                 victim->pkill, victim->pdeath, victim->mkill, victim->mdeath);
   send_to_char(buf, ch);

   send_to_char(line, ch);

   sprintf( buf, "  #yTempest : %3d  Red    : %3d  White   : %3d  Green  : %3d  Blue : %3d#n\n\r",
                 victim->spl[0], victim->spl[1], victim->spl[2],
                 victim->spl[3], victim->spl[4]);
   send_to_char( buf, ch );

   send_to_char(line, ch);

   sprintf( buf, "  #yUnarm : %4d  Slice : %4d  Stab  : %4d  Slash : %4d  Whip  : %4d#n\n\r",
                 victim->wpn[0], victim->wpn[1], victim->wpn[2], victim->wpn[3], victim->wpn[4]);
   send_to_char( buf, ch );

   sprintf( buf, "  #yClaw  : %4d  Blast : %4d  Pound : %4d  Crush : %4d  Grep  : %4d#n\n\r",
                 victim->wpn[5], victim->wpn[6], victim->wpn[7], victim->wpn[8], victim->wpn[9]);
   send_to_char( buf, ch );

   sprintf( buf, "  #yBite  : %4d  Pierc : %4d  Suck  : %4d#n\n\r",
