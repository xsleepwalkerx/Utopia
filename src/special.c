...
DECLARE_SPEC_FUN(	spec_guard	);
DECLARE_SPEC_FUN(	spec_kguard_two	);
DECLARE_SPEC_FUN(	spec_kguard	);
...
DECLARE_SPEC_FUN(	spec_omega		);
DECLARE_SPEC_FUN(	spec_rinoa		);
DECLARE_SPEC_FUN(	spec_ninja		);

int get_kingdom_num( CHAR_DATA *ch )
{
   if ( !IS_NPC(ch) )    
      return -1;

   if ( ch->pIndexData->vnum == 2500 || ch->pIndexData->vnum == 2501 || ch->pIndexData->vnum == 2503
        || ch->pIndexData->vnum == 2502 )
      return 1;

   if ( ch->pIndexData->vnum == 4500 || ch->pIndexData->vnum == 4501 || ch->pIndexData->vnum == 4503
        || ch->pIndexData->vnum == 4502 || ch->pIndexData->vnum == 4507 )
      return 2;

   if ( ch->pIndexData->vnum == 3500 || ch->pIndexData->vnum == 3501 || ch->pIndexData->vnum == 3503
        || ch->pIndexData->vnum == 3502 )
      return 3;

   if ( ch->pIndexData->vnum == 16500 || ch->pIndexData->vnum == 16501 || ch->pIndexData->vnum == 16503
        || ch->pIndexData->vnum == 16502 )
      return 4;

   return -1;
}

bool is_same_kingdom( CHAR_DATA *ch, CHAR_DATA *victim )
{
   if ( !IS_NPC(ch) || IS_NPC(victim) || endozone > 0 )
      return FALSE;

   if ( victim->pcdata->kingdom == 1 && ( ch->pIndexData->vnum == 2500 || ch->pIndexData->vnum == 2501
        || ch->pIndexData->vnum == 2502 ) )
      return TRUE;

   if ( victim->pcdata->kingdom == 2 && ( ch->pIndexData->vnum == 4500 || ch->pIndexData->vnum == 4501
        || ch->pIndexData->vnum == 4502 || ch->pIndexData->vnum == 4507 ) )
      return TRUE;

   if ( victim->pcdata->kingdom == 3 && ( ch->pIndexData->vnum == 3500 || ch->pIndexData->vnum == 3501
        || ch->pIndexData->vnum == 3502 ) )
      return TRUE;

   if ( victim->pcdata->kingdom == 4 && ( ch->pIndexData->vnum == 16500 || ch->pIndexData->vnum == 16501
        || ch->pIndexData->vnum == 16502 ) )
      return TRUE;
...
void capture_person( CHAR_DATA *ch, CHAR_DATA *victim )
{
   char buf[MAX_INPUT_LENGTH];

   if ( IS_NPC( victim ) || victim->pcdata->capture_timer > 0 )
      return;

   if ( victim->in_room->vnum >= 550 && victim->in_room->vnum < 629 )
      return;

   act( "You viciously grab $N by the collar as you lift $M in the air.", ch, NULL, victim, TO_CHAR );
   act( "$n viciouslys grab you by the collar as $e lifts you in the air.", ch, NULL, victim, TO_VICT );
   act( "$n viciouslys grab $N by the collar as $e lifts $M in the air.", ch, NULL, victim, TO_NOTVICT );
   act( "You sneer at $N '#yInto the dungeon you foul scoundrel!", ch, NULL, victim, TO_CHAR );
   act( "$n sneers at you '#yInto the dungeon you foul scoundrel!", ch, NULL, victim, TO_VICT );
   act( "$n sneers at $N '#yInto the dungeon you foul scoundrel!", ch, NULL, victim, TO_NOTVICT );
   act( "A blazing carriage rushes by you as you throw $N into it!", ch, NULL, victim, TO_CHAR );
   act( "A blazing carriage rushes by $n as $e throws you into it!", ch, NULL, victim, TO_VICT );
   act( "A blazing carriage rushes by $n as $e throws $N into it!", ch, NULL, victim, TO_NOTVICT );
   act( "You scream loudly as you are dragged to the execution room!", victim, NULL, NULL, TO_CHAR );
   act( "You hear alot of screaming as $n is dragged away to $s upcoming death!", victim, NULL, NULL, TO_ROOM );

   if ( IS_NPC( ch ) )
      sprintf(buf,"#w%s #yhas been #P#zcaptured#n #yby #w%s#y!#n",victim->pcdata->switchname,ch->short_descr);
      else sprintf(buf,"#w%s #yhas been #P#zcaptured#n #yby #w%s#y!#n",victim->pcdata->switchname,ch->pcdata->switchname);

   char_from_room( victim );

   if ( !IS_NPC( ch ) )
      char_to_room( victim, get_room_index( king_table[ch->pcdata->kingdom].altar ) );
      else char_to_room( victim, get_room_index( king_table[get_kingdom_num( ch )].altar ) );

   if ( victim->pcdata->clanrank < 1 )
      victim->pcdata->clanrank = 1;

   if ( !IS_NPC( ch ) && ch->pcdata->clanrank <= victim->pcdata->clanrank && ch->pcdata->clanrank < 6 )
      {
          ch->pcdata->clanrank++;
          ch->max_hit += 400000;
          stc( "#yYour #Pranking #win the #Rkingdom #whas #Cimproved#w.#n\n\r", ch );

          if ( !armageddon && victim->pcdata->clanrank > 1 )
             {
                victim->max_hit -= 400000;
                victim->pcdata->clanrank = UMAX( 1, victim->pcdata->clanrank - 1 );
                stc( "#yYour #Pranking #win the #Rkingdom #whas #eworsen#w.#n\n\r", victim );
...
void do_capture( CHAR_DATA *ch, char *argument )
{
   ...
   if ( arg[0] == '\0' )
   {
      stc("Whom do you wish to capture?\n\r",ch);
      return;
   }
   ...
   {
      stc("You can't capture non-kingdom people.\n\r",ch);
      return;
   }

   if ( victim->pcdata->capture_timer > 0 )
   {
      stc("They are already captured.\n\r",ch);
      return;
   }

   if ( victim->position != POS_MORTAL && victim->position != POS_INCAP )
   {
      stc("But they aren't morted.\n\r",ch);
      return;
   }

   if ( ch->in_room->vnum >= 550 && ch->in_room->vnum < 629 )
   {
      stc("You can't capture people in the arena.\n\r",ch);
      return;
   }
...
void do_release( CHAR_DATA *ch, char *argument )
{
   ...
   if ( victim->pcdata->capture_timer == 0 )
   {
      stc("But they aren't captured.\n\r",ch);
      return;
   }

   if ( !IS_IMMORTAL(ch) && victim->pcdata->kingdom != ch->pcdata->kingdom )
   {
      stc("You aren't in the same kingdom as them.\n\r",ch);
      return;
   }

   act( "You quietly sneak up to $N, removing the chains which are weighing $M down.", ch, NULL, victim, TO_CHAR );
   act( "$n quietly sneaks up to you, removing the chains which are weighing you down.", ch, NULL, victim, TO_VICT );
   act( "$n quietly sneaks up to $N, removing the chains which are weighing $M down.", ch, NULL, victim, TO_NOTVICT );

   act( "You release $N from $S bonds, freeing $M.", ch, NULL, victim, TO_CHAR );
   act( "$n releases you from your bonds, freeing you.", ch, NULL, victim, TO_VICT );
   act( "$n releases $N from $S bonds, freeing $M.", ch, NULL, victim, TO_NOTVICT );
...
bool spec_vladd_decap( CHAR_DATA *ch)
{
   ...
   for ( victim = ch->in_room->people; victim != NULL; victim = v_next )
   {
       ...
       if ( IS_NPC(victim) || victim == ch || victim->pcdata->capture_timer > 2 || victim->pcdata->capture_timer == 0
            || IS_SET( victim->loc_hp[0], LOST_HEAD ) )
          continue;
       ...
       if ( rnd_chat <= 5 )
          sprintf(buf,  "Do I ever love my JOB!");
          else if ( rnd_chat <= 8 )
                  sprintf(buf,  "%s, You have just been #wF#RU#yB#eA#CR#PE#wD#n!!!", victim->name );
                  else sprintf(buf,  "All against the kingdom of %s #Pwill #eDIE#P!#n", king_table[get_kingdom_num( ch )].who_name);

       do_chat( ch, buf );
       sprintf(buf,"#C%s #whas #ybeen #R#zbeheaded#n #wby the #L%s#w.#n",victim->pcdata->switchname,ch->short_descr);
	 do_info(ch,buf);
      ...
      if ( victim->pcdata->kingdom != 0 )
         {
            ...
            king_table[get_kingdom_num( ch )].pk += 1;
         }
...
bool spec_kguard_two(CHAR_DATA *ch)
{
   DESCRIPTOR_DATA       *d;
   CHAR_DATA             *victim;
   char buf[MAX_STRING_LENGTH];
   int kingdom_num = get_kingdom_num(ch);
   int count;

   if (ch->fighting) return FALSE;

   if ( armageddon == TRUE || nogang == 10 || mysafe == 10 )
      return FALSE;
   ...
   for (d = descriptor_list; d != NULL; d = d->next)
   {
       if ( (victim = d->character) == NULL
        ...
        if ( is_same_kingdom(ch,victim) )
           continue;

        if ( victim->pcdata->capture_timer > 0 || victim->safe_timer > 0 )
           continue;
        ...
        if ( !IS_AFFECTED(ch, AFF_ETHEREAL) && IS_AFFECTED(victim, AFF_ETHEREAL) && victim->fight_timer == 0 )
           REMOVE_BIT(victim->affected_by, AFF_ETHEREAL);

        if ( !IS_AFFECTED(ch, AFF_SHADOWPLANE) && IS_AFFECTED(victim, AFF_SHADOWPLANE) && victim->fight_timer == 0 )
           REMOVE_BIT(victim->affected_by, AFF_SHADOWPLANE);

        if ( is_safe(ch,victim) )
           continue;

        if ( ch->in_room == victim->in_room && kingdom_num < 9 )
           {
              do_say( ch, "Halt! Who goes there?" );

              act( "$n looks at you in surprise as $e leaps back and draws out $s weapon.", ch, NULL, victim, TO_VICT );
              act( "$n looks at $N in surprise as $e leaps back and draws out $s weapon.", ch, NULL, victim, TO_NOTVICT );

              do_yell( ch, "Intruder Alert! Sound the alarm!!!" );
           }

        if ( ch->fight_timer == 0 )
           {
              sprintf( buf, "#C%s #wis #R#zinvading#n #wthe Kingdom of %s!#n",
                             victim->pcdata->switchname, king_table[get_kingdom_num( ch )].who_name);
              do_info( victim, buf );
           }
        ...
        if ( ch->in_room == victim->in_room )
           {
              ...
              do_yell( ch, "Intruder Alert! Sound the alarm!!!" );
                   ...
                   act( "$n charges into the room with sheer hatred in $s eyes.", ch, NULL, NULL, TO_ROOM );
                }
...
bool spec_ninja(CHAR_DATA *ch)
{
   ...
   for ( victim = ch->in_room->people; victim != NULL; victim = v_next )
   {
       ...
       if ( is_safe(ch,victim) || IS_NPC(victim) || victim == ch || victim->position > POS_INCAP )
          continue;
       ...
       if ( rnd_chat <= 5 )
          sprintf(buf,  "We have never met, but now is time for you to die....");
                  ...
                  sprintf(buf,  "%s, You have just been #wF#RU#yB#eA#CR#PE#wD#n!!!", victim->name );
                  else sprintf(buf,  "Long live the Taira!");

       do_chat( ch, buf );
	 sprintf(buf,"%s has been assassinated by %s.",victim->pcdata->switchname,ch->short_descr);
	 do_info(ch,buf);
   ...
   for ( d = descriptor_list; d != NULL; d = d->next )
   {
       if ( !IS_PLAYING(d)
        ...
        if ( ch->in_room->vnum < 3000 || ch->in_room->vnum > 3200 || ( victim->in_room != ch->in_room && number_percent() < 90 ) )
           continue;
           else {
                   char_from_room(ch);
                   char_to_room(ch,victim->in_room);
        ...
        act("You leap from the shadows placing your weapon into $N's back #r[500000]#n...\n\r",ch,NULL,victim,TO_CHAR);
        act("$n leaps from the shadows placing $s weapon into your back! #r[500000]#n\n\r",ch,NULL,victim,TO_VICT);
        act("$n leaps from the shadows placing $s weapon into $N's back! #r[500000]#n\n\r",ch,NULL,victim,TO_NOTVICT);
        victim->hit -= 499999;
        hurt_person(ch,victim,1,TRUE);
        return TRUE;
   }
...
bool spec_rinoa(CHAR_DATA *ch)
{
   ...
   for (d = descriptor_list; d != NULL; d = d->next)
   {
	   ...
	   || victim->in_room == NULL
           || victim->pcdata->chobj != NULL
           || victim->in_room->area != ch->in_room->area
           || !str_cmp(victim->name,"Lightmist")
           || !str_cmp(victim->name,"Purplerain") )
	{
            continue;
        }

        char_from_room(victim);
        char_to_room( victim, get_room_index(3054) );
...
bool spec_cast_adept( CHAR_DATA *ch )
{
    ...
    if ( ch->pIndexData->vnum >= 90000 && ch->pIndexData->vnum <= 90999 )
    {
       switch ( number_range( 1, 4 ) )
       {
          default:
          case 1: act( "", ch, NULL, NULL, TO_ROOM );
	            spell_holy_word( skill_lookup( "holy word" ), ch->level, ch, victim );
	            return TRUE;

          case 2: act( "", ch, NULL, NULL, TO_ROOM );
	            spell_alabaster_potion( skill_lookup( "alabaster potion" ), ch->level, ch, victim );
	            return TRUE;

          case 3: act( "", ch, NULL, NULL, TO_ROOM );
	            spell_divine_strength( skill_lookup( "divine strength" ), ch->level, ch, victim );
	            return TRUE;

          case 4: act( "", ch, NULL, NULL, TO_ROOM );
	            spell_llothbless( skill_lookup( "llothbless" ), ch->level, ch, victim );
	            return TRUE;
       }
...
bool spec_cast_cleric( CHAR_DATA *ch )
{
    ...
    if ( ch->position != POS_FIGHTING || ch->in_room == NULL )
	return FALSE;
...
rnd_chat = number_range (1, 10);
   ...
   if ( rnd_chat <= 5)
                sprintf( buf, "Your time has come %s...\n\r", victim->pcdata->switchname); 
   else if ( rnd_chat <= 6)
                sprintf( buf, "Time to die....\n\r");
   else if ( rnd_chat <= 7)
                sprintf( buf, "HAH!, a little message from Vladd......DIE!....\n\r");
   else if ( rnd_chat <= 8)
                sprintf( buf, "Welcome to your fate....\n\r");
   else if ( rnd_chat <= 9)
                sprintf(buf,  "I'm godzilla, your JAPAN!.........\n\r");
   else if ( rnd_chat <= 10)
...
const   struct  spec_type       spec_table      [ ] =
{
    ...
    { "spec_thief",             	0              },
	{ "spec_eater",			0 },
	{ "spec_gremlin_original",	0 },
	{ "spec_gremlin_born", 		0 },
	{ "spec_rogue", 		0 },
	{ "spec_zombie_lord", 		0 },
	{ "spec_guard_werewolf",	0 },
	{ "spec_guard_dragon",		0 },
	{ "spec_guard_vampire",		0 },
	{ "spec_guard_mage",		0 },
	{ "spec_guard_demon",		0 },
	{ "spec_guard_drow",		0 },
	{ "spec_guard_ninja",		0 },
	{ "spec_guard_monk",		0 },
	{ "spec_guard_highlander",	0 },
	{ "spec_kguard",	spec_kguard },
	{ "spec_ninja",	0 },
      { "spec_kguard_two", spec_kguard_two },
      { "spec_vladd_decap", spec_vladd_decap },
	{ "spec_guard_guardtwo",	0 },
	{ "spec_guard_guardthree",	0 },
	{ "spec_guard_guardfour",	0 },
	{ "spec_guard_guardfive",       0 },
      { "spec_guard_guardsix",       0 },
      { "spec_guard_guardseven",       0 },
      { "spec_guard_guardeight",       0 },
	{ "spec_omega",		0 },
	{ "spec_rinoa",		0 },
      { "spec_guard_guardnine",       0 },
      { "spec_guard_guardten",       0 },
        { "spec_assassin",		0 },
	{ "spec_guard_cyborg",		0},
        { "spec_executioner",           0 },    
/*
